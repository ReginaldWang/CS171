...
class LocalClock:
    ...
    def now(self):
        """Return current local clock with drift"""
        R_now = time.time()
        return self.L_base + (R_now - self.R_base) * (1 + self.rho)

    def adjust(self, offset):
        """Apply Cristian's algorithm offset"""
        self.L_base = self.now() + offset
        self.R_base = time.time()

async def sync_with_server(local_clock):
    """Send request to NW server, receive server time, adjust local clock"""
...
async def main(duration, epsilon_max, rho):
    ...
    with open("clock_log.csv", "w", newline="") as csvfile:
        ...
        while time.time() < end_time:
            ...
            print(f"Actual: {actual_time:.3f}, Local: {local_time:.3f}")

            if abs(local_time - actual_time) >= epsilon_max:
                await sync_with_server(local_clock)
                print("[SYNC] Clock synchronized with server!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epsilon", type=float, default=0.05, help="Max tolerable error")
    parser.add_argument("--rho", type=float, default=1e-6, help="Clock drift")
    args = parser.parse_args()

    asyncio.run(main(args.d, args.epsilon, args.rho))
